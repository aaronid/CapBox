...
class LigneFacture {
	...
	function findById($id_) {
		$sql = "select _ID, GROUPE_FACTURE, DESIGNATION, TTVA, QTE, UNITE, PUHT, NUMERO, RANG from " . $this->tableName . " where _ID = " . $id_;
		$query = mysql_query($sql);
		if (!$query) {
			die (mysql_error());
		}
		$this->id = mysql_result($query, 0, "_ID");
		$this->idGroupeFacture = mysql_result($query, 0, "GROUPE_FACTURE");
	...
	function duplicate($idGroupeFacture) {
		$dupLigne = new LigneFacture();
		$dupLigne->idGroupeFacture = $idGroupeFacture;
	...
	function insert() {
		$sql = "insert into " . $this->tableName . " (GROUPE_FACTURE, DESIGNATION, TTVA, QTE, UNITE, PUHT, NUMERO, RANG) values ('" . $this->idGroupeFacture . "', '" . str_replace("'", "''", $this->designation) . "', '" . $this->tauxTva . "', '" . $this->quantite . "', '" . $this->unite . "', '" . $this->prixUnitaireHT . "', " . $this->numero . ", '" . $this->rang . "')";
		mysql_query($sql);
	...
	function getHtmlId() {
		return $this->idGroupeFacture . "_" . $this->id;
	}
