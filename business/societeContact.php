...
class SocieteContact {
    ...
    /**
     * SELECT BY LOGIN - TABLE SOCIETECONTACT
     * Sélectionner une SOCIETECONTACT par son LOGIN utilisateur
     * @param $login
     * @return $this
     */
	function findByLogin($login) {
        ...
        try{
            $sql = "select _ID, ID_SOCIETE, CODE_PROFIL, LOGIN, CIVILITE, NOM, PRENOM, INITIALE, TEL_FIX, TEL_MOB, EMAIL, FONCTION, COMMENTAIRE from "
                . $this->tableName
                . " where LOGIN = '"
                . $login . "'";
            ...
            $societe = new Societe();
            $societe = $societe->findById($this->idSociete);
    ...
    /**
     * Création d'un objet SocieteContact à partir des données d'une BDD
     * @param $result
     */
	function itemBuilder($result){
        $this->id = $result->_ID;
        $this->idSociete = $result->ID_SOCIETE;
        $this->codeProfil = $result->CODE_PROFIL;
        $this->login = $result->LOGIN;
        $this->civilite = $result->CIVILITE;
        $this->nom = $result->NOM;
        $this->prenom = $result->PRENOM;
        $this->initiale = $result->INITIALE;
