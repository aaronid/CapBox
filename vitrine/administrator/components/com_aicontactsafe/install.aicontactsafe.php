...
// 2.0.2 modifications in database 
	...
	$db->setQuery( $query );
	$db->query();
	$query = 'ALTER TABLE `#__aicontactsafe_profiles` CHANGE `field_label` `field_label` text NOT NULL;';
	$db->setQuery( $query );
	$db->query();
	$query = 'ALTER TABLE `#__aicontactsafe_profiles` CHANGE `field_label_message` `field_label_message` text NOT NULL;';
	$db->setQuery( $query );
	$db->query();
...
// 2.0.6 modifications in database 
	$query = 'ALTER TABLE `jos_aicontactsafe_messages` CHANGE `email_replay` `email_reply` varchar(100) NOT NULL default \'\';';
	$db->setQuery( $query );
	$db->query();
	$query = 'ALTER TABLE `jos_aicontactsafe_messages` CHANGE `subject_replay` `subject_reply` text NOT NULL default \'\';';
	$db->setQuery( $query );
	$db->query();
	$query = 'ALTER TABLE `jos_aicontactsafe_messages` CHANGE `message_replay` `message_reply` text NOT NULL default \'\';';
	$db->setQuery( $query );
	$db->query();
