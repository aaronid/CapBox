...
class ZooWidget {
	...
	*/
	public function edit($id = null){
		...
		$form->addElementPath($this->zoo->path->path('joomla:elements'));
		$form->addElementPath($this->widgetkit['path']->path('plugin.root:elements'));
	...
	public function render($widget) {
		...
		if (isset($widget->zoo['params']) && $widget->type == $this->type) {
			...
			if ($application = $this->zoo->table->application->get($params->get('application', 0))) {
				...
				// load template
				if (($zoo_items = $this->zoo->zoomodule->getItems($params)) && !empty($zoo_items)) {
