	...
	*/
	public function site() {
		...
		if ($this->options->get('mediaplayer_enable', 1)) {
			...
			$this['asset']->addFile('js', 'mediaplayer:mediaelement/mediaelement-and-player.js');
			$this['asset']->addString('js', sprintf("jQuery(function($){ mejs.MediaElementDefaults.pluginPath='".$pluginPath."'; $('%s').mediaelementplayer(%s); });", $this->options->get('mediaplayer_selector', 'video,audio'), count($options) ? json_encode($options) : '{}'));
		}
