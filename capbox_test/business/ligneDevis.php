...
class LigneDevis {
	...
	function LigneDevis() {
		$this->tableName = "ligne_devis";
		$this->quantite = "1.00";
		$this->prixUnitaireHT = "0.00";
	}

	function findById($id_) {
		$sql = "select _ID, GROUPE_DEVIS, DESIGNATION, TTVA, QTE, UNITE, PUHT, NUMERO, RANG from " . $this->tableName . " where _ID = " . $id_;
		$query = mysql_query($sql);
		if (!$query) {
			die (mysql_error());
		}
		$this->id = mysql_result($query, 0, "_ID");
		$this->idGroupeDevis = mysql_result($query, 0, "GROUPE_DEVIS");
	...
	function duplicate($idGroupeDevis) {
		$dupLigne = new LigneDevis();
		$dupLigne->idGroupeDevis = $idGroupeDevis;
	...
	function insert() {
		$sql = "insert into " . $this->tableName . " (GROUPE_DEVIS, DESIGNATION, TTVA, QTE, UNITE, PUHT, NUMERO, RANG) values ('" . $this->idGroupeDevis . "', '" . str_replace("'", "''", $this->designation) . "', '" . $this->tauxTva . "', '" . $this->quantite . "', '" . $this->unite . "', '" . $this->prixUnitaireHT . "', " . $this->numero . ", '" . $this->rang . "')";
		mysql_query($sql);
	...
	function getHtmlId() {
		return $this->idGroupeDevis . "_" . $this->id;
	}
