<?php
	...
	$acompte = new Acompte();
	if ($action == "add") {
		$acompte->idFacture = $idFacture;
	} elseif ($action == "load") {
		if (isset($_POST['ID_ACOMPTE']) && !empty($_POST['ID_ACOMPTE'])) {
			$acompte->findById($_POST['ID_ACOMPTE']);
		} else {
			$acompte->idFacture = $idFacture;
		}
	} elseif ($action == "save") {
		...
		$acompte->montant = str_replace(',', '.', $_POST['MONTANT']);
		$acompte->dateEmission = date_create_from_format('d/m/Y', $_POST['DATE_ACOMPTE']);
		$acompte->commentaire = $_POST['COMMENTAIRE'];

		if (isset($_POST['ID_ACOMPTE']) && !empty($_POST['ID_ACOMPTE'])) {
			$acompte->id = $_POST['ID_ACOMPTE'];
			$acompte->update();
		} else {
			$acompte->insert();
		}
	...
	$acompte = Acompte::findByIdFacture($idFacture);
	$urlBase = "acompte_facture_pop.php?idFacture=" . $idFacture;
...
<head>
    ...
    <script type="text/javascript">
        function selectAvoir(line, idAvoir) 
        {
            ...
            document.getElementById("ID_ACOMPTE").value = idAvoir;
        }
        ...
        function isValidateData() {
        	var validate = true;
        	var dateAvoir = document.getElementById("DATE_ACOMPTE").value;
		...
		<table>
			<tbody>
                ...
                <tr>
	                <td class="searchTD">
	                	<span>Date</span>
						<input id="DATE_ACOMPTE" name="DATE_ACOMPTE" class="searchInput" type="text" title="Date" size="15" value="<?php echo date_format($avoirMod->dateEmission, 'd/m/Y'); ?>" /> 
	                </td>
	                <td class="searchTD">
	                	<span>Montant</span>
                ...
                <tr>
	                <td>
						<input id="ID_ACOMPTE" name="ID_ACOMPTE" type="hidden" value="<?php echo $avoirMod->id; ?>" />
