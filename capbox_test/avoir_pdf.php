...
//			$re = mysql_fetch_array($resul);
			...
			$this->Cell(90, 10, 'AVOIR', 0, 1, 'C', true);
			//Saut de ligne
			$this->SetFont('Arial', '', 9);
			$this->Ln(2);
			//sous titre
			$d1= utf8_decode('Date émission : ' . date('d/m/Y', strtotime($avoir->dateEmission)));
			if (!empty($avoir->idFacture)) {
				$d2=utf8_decode('Référence facture : ' . $avoir->idFacture);
			} else {
				$d2=utf8_decode('Aucune facture de référence');
			}
			$this->Cell(100);
			$this->Cell(90, 5, 'Ref : ' . $avoir->reference, 0, 0, 'R');
			$this->Ln(1);
			$this->Cell(100);
			...
			if($pag !=1){
				...
				//TITRE
				$this->Ln(5);
				$this->Cell(0, 10, utf8_decode($avoir->titre) . ' (suite)', 1, 0, 'C');
				$this->Ln(10);
				//En-tête du tableau
...
//	$re = mysql_fetch_array($resul);
	...
	$socClient->findByLogin($avoir->idSocieteClient);
//	$sel = "select * from contact where _ID = '" . $avoir->destinataire . "'";
//	$resul = mysql_query($sel);
//	$re = mysql_fetch_array($resul);
	...
	$contactAvoir = new SocieteContact();
	$contactAvoir->findById($avoir->idSocieteContact);
//	$selectinter = "select * from interlocuteur where _ID = '" . $avoir->idInterlocuteur . "'";
//	$resultinter = mysql_query($selectinter);
//	$rinter = mysql_fetch_array($resultinter);
	
	$pdf->Cell(0, 10, 'Votre interlocuteur : ' . utf8_decode($contactAvoir->prenom) . ' ' . utf8_decode($contactAvoir->nom), 0, 0, 'L');
	$pdf->Ln();

	//TITRE
	$pdf->MultiCell(0, 5, utf8_decode($avoir->titre), 1, 'C', false);
	$pdf->Ln(10);
	
	//TABLEAU : LISTING ligne_avoir : couleurs, épaisseur du trait et police grasse
	$pdf->SetFillColor(230, 230, 230);
	$pdf->SetDrawColor(0, 0, 0);
	$pdf->SetLineWidth(.3);
	...
	$pdf->Ln();
	$pdf->SetFont('Arial', '', 7);
	$chaine = str_replace('€', 'euros', $avoir->commentaire);
	...
	//$pdf->Ln(5);
	if (!empty($avoir->remise) && $avoir->remise != 0) {
		$pdf->Cell(115);
		$pdf->Cell(51, 5, 'TOTAL HT :', 0, 0, 'L', false);
		$pdf->Cell(24, 5, $avoir->getHTPriceFormat(), 0, 0, 'R');
		$pdf->Ln();
//		$remise=$tll * $rd['REMISE'] / 100;
//		$remise=number_format($remise, 2, ',', ' ');
		$pdf->Cell(115);
		$pdf->Cell(51, 5, 'Remise globale ' . $avoir->getRemiseFormat() . ' % :', 0, 0, 'L');
		$pdf->Cell(24, 5, $avoir->getTotalRemiseFormat(), 0, 0, 'R');
		$pdf->Ln();
		$pdf->Cell(115);
		$pdf->Cell(51, 5, 'TOTAL HT NET :', 0, 0, 'L', true);
		$pdf->Cell(24, 5, $avoir->getHTPriceRemiseFormat(), 0, 0, 'R', true);
		$pdf->Ln();
	}else{
		$pdf->Cell(115);
		$pdf->Cell(51, 5, 'TOTAL HT :', 0, 0, 'L');
		$pdf->Cell(24, 5, $avoir->getHTPriceRemiseFormat(), 0, 0, 'R');
		$pdf->Ln();
	}
	...
	foreach ($allTva as $tva) {
		if ($tva->id != 0.0 && ($tva->isActif || !empty($usedTva[$tva->id]))) {
			...
			$pdf->Cell(24, 5, $avoir->getTvaPriceFormat($tva->id), 0, 0, 'R');
			$pdf->Ln();
		}
	}
	...
	$pdf->Cell(24, 5, $avoir->getTTCPriceFormat(), 0, 0, 'R', true);
	$pdf->Ln(10);
	$pdf->Cell(115);
