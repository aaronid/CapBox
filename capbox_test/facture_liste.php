<?php
	...
	if(isset($_GET['print'])){
		...
		while ($val=mysql_fetch_array($result)) {
			...
			$csv .= ';'.$factureEdit->getTTCPriceFormat().';'.$factureEdit->getTotalRemiseFormat().';'.$factureEdit->getTotalAcompteFormat().';'. utf8_decode($val["VALIDATION"]).';'. utf8_decode($val["DATE_VALIDITE"]).';'. utf8_decode($str)."\n";
		} 
		print($csv); 
		exit; 
	}
