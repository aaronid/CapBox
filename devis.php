<?php
	...
	if (!empty ($_POST)) {
		$currentDevis = $_SESSION['currentDevis'];
		// METTRE A JOUR LE DEVIS
		$currentDevis->reference = $_POST['reference'];
		$currentDevis->idSociete = $societeContact->societe->id;
		$currentDevis->idSocieteClient = $_POST['destinataire'];
		$currentDevis->dateEmission = implode('/', array_reverse(explode('/', $_POST['date_emission'])));
		$currentDevis->dateValidite = '';
		
		$currentDevis->titre = $_POST['titre'];
		$currentDevis->idSocieteContact = $_POST['interlocuteur'];
		$currentDevis->remise = $_POST['Hremise1'];
		$currentDevis->acompte = $_POST['Hacompte'];
		$currentDevis->commentaire = $_POST['commentaire'];
		...
		if (endsWith($currentDevis->reference, ($societeContact->societe->refDevisIncre + 1))) {
			$societeContact->societe->refDevisIncre++;
			$societeContact->societe->update();
		}
		
		foreach ($currentDevis->groupes as $groupe) {
			if (isset($_POST['idGrp' . $groupe->id])) {
				$groupe->designation = $_POST['designationGrp' . $groupe->id];
				if ($isDuplication) {
					$dupGroupe = $groupe->duplicate($currentDevis->id);
					$currentDevis->groupes[$dupGroupe->id] = $dupGroupe;
				}
				else {
					$groupe->update();
				}
		...
		foreach ($newGroupes as $newIdGroupe) {
			if (isset($_POST['idGrp' . $newIdGroupe])) {
				$newGroupe = $currentDevis->addGroupe();
				$newGroupe->designation = $_POST['designationGrp' . $newIdGroupe];
				$newGroupe->insert();
				$currentDevis->groupes[$newGroupe->id] = $newGroupe;
				foreach ($newLines as $newIdLine) {
					if (isset($_POST['idLine' . $newIdLine]) && 
						...
						<div class="art-post">
								...
								<div class="art-post-body">
								  <div class="art-post-inner art-article">
									<h2 class="art-postheader"> <img src="images/postheadericon.png" width="26" height="26" alt="postheadericon" /> Edition de votre devis </h2>
									<form id="frm" name="frm" action="devis.php?<?php if (!empty($id)) { echo "id=" . $id; } ?><?php if (isset($_GET['dup']) && !empty($_GET['dup'])) { echo "dup=" . $_GET['dup']; } ?>" method="post">
...
<table width="100%">
  <tbody>
	<tr>
	  <td height="32" colspan="2" style="background-color:#F0F0F0; background:images/fd_nav_ss_menu.png;" class="componentheading"><span style="font-weight: bold"><a href="#" onclick="document.frm.submit()"><img src="images/icones/enregistrer-page-icone-7705-32.png" width="24" height="24" />Sauvegarder </a>
		<img src="images/menuseparator.png" width="1" height="24" />	<a href="devis.php?<?php if (!empty($id)) { echo "id=" . $id; } ?><?php if (isset($_GET['dup']) && !empty($_GET['dup'])) { echo "dup=" . $_GET['dup']; } ?>"><img src="images/icones/supprimer-la-page-icone-9859-32.png" width="24" height="24" />Effacer la saisie en cours</a><img src="images/menuseparator.png" width="1" height="24" />  <a href="devis_liste.php"><img src="images/icones/demande-afficher-la-liste-icone-8709-32.png" width="24" height="24" /> Retour à la liste des devis</a></span></td>
		<?php
			if (!empty($_GET['dup'])) {
				$id = $_GET['dup'];
			}
			...
			if (empty($id)) {
				$newGrp = $devis->addGroupe();
				$newGrp->id = "New0";
				$devis->groupes[$newGrp->id] = $newGrp;
	...
	<tr>
	  <td width="50%" height="32" bgcolor="#F9F9F9" class="componentheading"><strong >		DEVIS N° : </strong>
		<input name="reference" type="text" id="reference"
		<?php
			if (!empty($devis->reference) and empty($_GET['dup'])) {
				echo "value=\"".$devis->reference."\"";
			}
			else{
				if (empty($societeContact->societe->refDevisPrefix) && $societeContact->societe->refDevisIncre > 0) {
					$ref1 = mysql_query("select _ID from devis where ID_SOCIETE = " . $societeContact->societe->id);
					$ref2 = mysql_num_rows($ref1);
					$ref2++;
					echo "value=\"".$ref2."\"";
				} else {
					echo "value=\"" . $societeContact->societe->refDevisPrefix . ($societeContact->societe->refDevisIncre + 1) . "\"";
				}
			}
		?> size="10" />
	  ...
	  <td width="50%" height="32" align="right" bgcolor="#F9F9F9"><strong><img src="images/icones/icon-date.gif" width="16" height="16" /> Date de création du devis</strong> :
		<input name="date_emission" id="date_emission" tabindex="10" title="Saisir une date de document en cliquant sur cette zone." value="<?php
			if(empty($devis->dateEmission)){
				echo date('d/m/Y');
			}
			else{
				echo date('d/m/Y', strtotime($devis->dateEmission));
			}
			?>" size="10" alt="date" />
		</td>
	</tr>
	<tr>
	  <td valign="top" bgcolor="#F9F9F9"><p><strong>Intitulé du devis :</strong><br />
		  <textarea title="Saisir l'objet principal du devis (facultatif)." id="titre" name="titre" cols="70" rows="0" tabindex="11"><?php echo $devis->titre; ?></textarea>
	  </p></td>
	  <td bgcolor="#F9F9F9"><table width="100%" cellspacing="0" cellpadding="3">
		  ...
		  <tr>
			...
			<?php
				$selectioncontact = "";
				if(!empty($id)){
					$selectioncontact = $devis->idSocieteClient;
			...
			?>
			<div id="c_societe"> <?php echo $selClient->entreprise; ?></div><div id="c_nom"><?php echo $selClient->prenom." ".$selClient->nom; ?></div><div id="c_adresse"><?php echo $selClient->adresse1." - ".$selClient->adresse2; ?></div><div id="c_cp"><?php echo $selClient->codePostal." ".$selClient->ville; ?></div>
			<input type="hidden" name="destinataire" id="destinataire" value="<?php echo $selectioncontact; ?>" />			</td>
			<td>&nbsp;</td>
		  </tr>
		  <tr>
			<td>&nbsp;</td>
...
<table width="100%" id="tableArticle">
	...
	<?php
			foreach ($devis->groupes as $groupe) {
	?>
			<tr>
		  ...
		  <tbody>
			<tr>
			  <td width="70%" valign="top" bgcolor="#F9F9F9"><!-- Pied gauche document -->
					...
					<p><strong>Commentaires ajoutés au devis :</strong></p>

					  <textarea title="Saisir l'objet principal du devis (facultatif)." id="commentaire" name="commentaire" cols="85" rows="5" tabindex="11"><?php echo $devis->commentaire; ?></textarea>
				  ...
				  <div id="divTotalCumuleE">
					<table width="100%" cellpadding="3">
					  <tbody>
						...
						<tr>
						  <td width="60%" bgcolor="#EAF0F7">
						  	<strong> SOUS-TOTAL HT </strong>
						  </td>
						  <td align="right" bgcolor="#EAF0F7">
						  	<strong>
						  		<div id="totalht"><?php echo $devis->getHTPriceFormat(); ?></div>
						  		<input id="Htotalht" name="Htotalht" type="hidden" value="<?php echo $devis->getHTPrice() ?>"/>
						  	</strong>
						  </td>
						</tr>
						<tr>
						  <td bgcolor="#EAF0F7">Remise Globale
							<input title="Saisir le taux de la remise globale." id="remise1" name="remise1" alt="montant" size="10" value="<?php if(!empty($devis->remise)){ echo $devis->getRemiseFormat();}else{ echo '0,00';} ?>" onchange="calcul('r', '0');" tabindex="16" />
							<input id="Hremise1" name="Hremise1" type="hidden" value="<?php if(!empty($devis->remise)){ echo $devis->remise;}else{ echo '0';} ?>" />
							%</td>
						  <td align="right" bgcolor="#EAF0F7">
						  	<div id="remise2"></div>
						  	<input id="Hremise2" name="Hremise2" type="hidden"/>
						  </td>
						</tr>
						<tr>
						  <td bgcolor="#5289BA">
						  ...
						  <td align="right" bgcolor="#5289BA">
						  	<strong>
						  		<div class="Style1" id="totalht1"><?php echo $devis->getHTPriceRemiseFormat(); ?></div>
						  		<input id="Htotalht1" name="Htotalht1" type="hidden" value="<?php echo $devis->getHTPriceRemise(); ?>"/>
						  	</strong>
						  </td>
						</tr>
	<?php
	...
	?>
						<tr>
						  ...
						  <td align="right" bgcolor="#EAF0F7">
						  	<div id="tva<?php echo $tva->id; ?>"><?php echo $devis->getTvaPriceFormat($tva->id); ?></div>
						  	<input id="Htva<?php echo $tva->id; ?>" name="Htva<?php echo $tva->id; ?>" type="hidden" value="<?php echo $devis->getTvaPrice($tva->id); ?>"/>
						  </td>
						</tr>
	<?php
					}
	...
	?>
						<tr>
						  <td bgcolor="#5289BA">
						  	<span class="Style1"><strong>TOTAL Net TTC</strong></span>
						  </td>
						  <td align="right" bgcolor="#5289BA">
						  	<strong>
						  		<div class="Style1" id="totalttc"><?php echo $devis->getTTCPriceFormat(); ?></div>
						  		<input id="Htotalttc" name="Htotalttc" type="hidden" value="<?php echo $devis->getTTCPrice(); ?>"/>
						  	</strong>
						  </td>
						</tr>
					  </tbody>
			...
			<tr>
			  <td valign="top" bgcolor="#F9F9F9">
				<p class="componentheading"><strong>Modalités de paiement </strong>:</p>
				<p><img src="images/icones/argent-icone-6943-32.png" alt="" width="32" height="32" /> Acompte :
					<input title="Saisir le taux de l'acompte." id="acompte" name="acompte" alt="montant" size="10" value="<?php echo $devis->getAcompteFormat(); ?>" onchange="calcul('a','0');" tabindex="16" />
					<input type="hidden" name="Hacompte" id="Hacompte" value="<?php echo $devis->acompte; ?>" />
				<div style="display:inline" id="acompte2"><strong>% soit <?php echo $devis->getAcomptePriceTTCFormat(); ?>€ TTC</strong></div></p>
