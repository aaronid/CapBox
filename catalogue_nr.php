<?php 
	...
	if (!empty($_POST)) {
		...
		if (empty($id)) {
			$req="INSERT INTO cataloguep (FAMILLE,FOURNISSEUR,MARQUE,REF_FABRICANT,REF_DISTRIBUTEUR,PRIX_AU,PRIX_NET,PRIX_BASE,DESIGNATION,CLIENT) values('$FAMILLE','$FOURNISSEUR','$MARQUE','$REF_FABRICANT','$REF_DISTRIBUTEUR','$PRIX_AU','$PRIX_NET','$PRIX_BASE','$DESIGNATION','$client')";
		} else {
			$req="update cataloguep set FAMILLE='$FAMILLE', FOURNISSEUR='$FOURNISSEUR', MARQUE='$MARQUE', REF_FABRICANT='$REF_FABRICANT', REF_DISTRIBUTEUR='$REF_DISTRIBUTEUR', PRIX_AU='$PRIX_AU', PRIX_NET='$PRIX_NET', PRIX_BASE='$PRIX_BASE', DESIGNATION='$DESIGNATION' where _ID='$id'";
		}
		mysql_query($req);
  ...
  <tbody>
    ...
    <tr>
      <td width="30%" bgcolor="#F9F9F9" class="componentheading" style="font-weight: bold">Famille <strong>
        <select name="FAMILLE" id="FAMILLE">
          <?php 
		$select="(select distinct FAMILLE from cataloguep where CLIENT='$client') union (select distinct FAMILLE from catalogue) order by FAMILLE";
		$result=mysql_query($select);
		while($val=mysql_fetch_array($result)){
		
		?>
          <option value="<?php echo $val['FAMILLE']; ?>" <?php if($val['FAMILLE']==$fetch['FAMILLE']){ echo "selected"; } ?>><?php echo $val['FAMILLE']; ?></option>
          <?php } ?>
        </select>
        </strong></td>
      ...
      <td  bgcolor="#F9F9F9"><span style="font-weight: bold">Fournisseur</span><strong>
        <select name="FOURNISSEUR" id="FOURNISSEUR"><option ></option>
          <?php 
		$select="(select distinct FOURNISSEUR from cataloguep where CLIENT='$client') union (select distinct FOURNISSEUR from catalogue) order by FOURNISSEUR";
		$result=mysql_query($select);
		while($val=mysql_fetch_array($result)){
		?>
          <option value="<?php echo $val['FOURNISSEUR']; ?>"<?php if($val['FOURNISSEUR']==$fetch['FOURNISSEUR']){ echo "selected"; } ?>><?php echo $val['FOURNISSEUR']; ?></option>
          <?php } ?>
        </select>
    ...
    <tr>
      <td bgcolor="#F9F9F9" class="componentheading" style="font-weight: bold">Marque         <strong>
        ...
        <?php 
		$select="(select distinct MARQUE from cataloguep where CLIENT='$client') union (select distinct MARQUE from catalogue)  order by MARQUE";
		$result=mysql_query($select);
		while($val=mysql_fetch_array($result)){
		?>
          <option value="<?php echo $val['MARQUE']; ?>"<?php if($val['MARQUE']==$fetch['MARQUE']){ echo "selected"; } ?>><?php echo $val['MARQUE']; ?></option>
          <?php } ?>
        </select>
