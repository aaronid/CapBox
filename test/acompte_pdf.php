<?php
	...
	class PDF extends FPDF
	{
		...
		function Header()
		{
			...
			$this->Cell(90, 10, "FACTURE D'ACOMPTE", 0, 1, 'C', true);
			//Saut de ligne
			$this->SetFont('Arial', '', 9);
			$this->Ln(2);
			//sous titre
			$d1= utf8_decode('Date émission : ' . date('d/m/Y', strtotime($acompte->dateEmission)));
			if (!empty($acompte->idFacture)) {
				$facture = new Facture();
				$facture->findById($acompte->idFacture);
				$d2=utf8_decode('Référence facture : ' . $facture->reference);
			} else {
				$d2=utf8_decode('Aucune facture de référence');
			}
			$this->Cell(100);
			$this->Cell(90, 5, 'Ref : ' . $acompte->reference, 0, 0, 'R');
			$this->Ln(1);
			$this->Cell(100);
			...
			if($pag !=1){
				...
				//TITRE
				$this->Ln(5);
				$this->Cell(0, 10, utf8_decode($acompte->titre) . ' (suite)', 1, 0, 'C');
				$this->Ln(10);
				//En-tête du tableau
	...
	$pdf->Ln();
	$pdf->SetFont('Arial', '', 7);
	$chaine = str_replace('€', 'euros', $acompte->commentaire);
	...
	if (count($allAcompte) > 1) {
		$pdf->Cell(115);
		$pdf->Cell(51, 5, utf8_decode('Autre(s) acompte(s) :'), 0, 0, 'L');
		$pdf->Ln();
		foreach ($allAcompte as $otherAcompte) {
			if ($otherAcompte->id <> $id) {
				$pdf->Cell(115);
				$pdf->Cell(51, 5, utf8_decode($otherAcompte->reference), 0, 0, 'L');
				$pdf->Cell(24, 5, $otherAcompte->getMontantFormat(), 0, 0, 'R');
				$pdf->Ln();
			}
		}
		$pdf->Ln(5);
	...
	$pdf->Cell(115);
	$pdf->Cell(51, 5, utf8_decode('Acompte à règler :'), 0, 0, 'L', true);
	$pdf->Cell(24, 5, $acompte->getMontantFormat(), 0, 0, 'R', true);
	$pdf->Ln(10);
