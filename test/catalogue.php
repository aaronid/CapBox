<?php 
	...
	if (!empty($_POST)) {
		...
		if (empty($id)) {
			$req="INSERT INTO cataloguep (FAMILLE,SOUS_FAMILLE,FOURNISSEUR,MARQUE,REF_FABRICANT,REF_DISTRIBUTEUR,PRIX_AU,PRIX_NET,PRIX_BASE,DESIGNATION,ID_SOCIETE) values('$FAMILLE','$SOUS_FAMILLE','$FOURNISSEUR','$MARQUE','$REF_FABRICANT','$REF_DISTRIBUTEUR','$PRIX_AU','$PRIX_NET','$PRIX_BASE','" . str_replace("'", "''", $DESIGNATION) . "'," . $societeContact->societe->id . ")";
		} else {
			$req="update cataloguep set FAMILLE='$FAMILLE',SOUS_FAMILLE='$SOUS_FAMILLE',FOURNISSEUR='$FOURNISSEUR', MARQUE='$MARQUE', REF_FABRICANT='$REF_FABRICANT', REF_DISTRIBUTEUR='$REF_DISTRIBUTEUR', PRIX_AU='$PRIX_AU', PRIX_NET='$PRIX_NET', PRIX_BASE='$PRIX_BASE', DESIGNATION='" . str_replace("'", "''", $DESIGNATION) . "' where _ID='$id'";
		}
		mysql_query($req);
