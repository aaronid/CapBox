<?php
	...
	// Suppression avoir
	if (isset($_GET['avoir_sup'])) {
		$avoir = new Avoir();
		$avoir->findById($_GET['avoir_sup']);
		$avoir->delete();
	}
	
	// Validation avoir
	if (isset($_GET['avoir_val'])) {
		$avoir = new Avoir();
		$avoir->findById($_GET['avoir_val']);
		$avoir->validate();
	}
	...
	// Edition CSV de l'avoir
	if (isset($_GET['print'])) {
		header("Content-type: application/vnd.ms-excel"); 
		$inputFile = $societeContact->societe->id . "_avoirs.csv";
		...
		$select = "select * from avoir WHERE ID_SOCIETE = " . $societeContact->societe->id . " " . $quete;
		...
		while ($val=mysql_fetch_array($result)) {
			...
			$avoirEdit = new Avoir();
			$avoirEdit->findById($val['_ID']);

			$csv .= utf8_decode($interloc2["INITIALE"]).';'. utf8_decode($val["REFERENCE"]).';'. utf8_decode($destinat).';'. utf8_decode($val["DATE_EMISSION"]).';'. utf8_decode($titre).';'. $avoirEdit->getHTPriceFormat(); // le \n final entre " " 
			foreach ($allTva as $tva) {
				$csv .= ';'.$avoirEdit->getTvaPriceFormat($tva->id);
			}
			$csv .= ';'.$avoirEdit->getTTCPriceFormat().';'.$avoirEdit->getTotalRemiseFormat().';'.$avoirEdit->getAcompteFormat().';'. utf8_decode($val["VALIDATION"]).';'. utf8_decode($val["DATE_VALIDITE"]).';'. utf8_decode($str)."\n";
		} 
		print($csv); 
		exit; 
	}
...
<head>
	...
	function ex(avoir)
	{
		var x=confirm("Etes-vous sûr de supprimer cet avoir ?")
		if (x)
			window.location = "avoir_liste.php?avoir_sup=" + avoir;
	}
	function fex(avoir)
	{
		var x=confirm("Etes-vous sûr de valider cet avoir ?")
		if (x)
			window.location = "avoir_liste.php?avoir_val=" + avoir;
	}
	function zex(avoir)
	{
		var x=confirm("Etes-vous sûr de valider cet avoir ?")
		if (x)
			window.location = "avoir.php?id=" + avoir;
	}
	function go() 
	{
...
<body>
	...
	<div id="art-main">
								...
								<div class="art-post-body">
									<div class="art-post-inner art-article">
										<h2 class="art-postheader">
											<img src="images/postheadericon.png" width="26" height="26" alt="postheadericon" /> Liste de vos avoirs
										</h2>
																	...
																	<td height="32" background="images/fd_nav_ss_menu.png" bgcolor="#F0F0F0" colspan="3">
																		<span style="font-weight: bold">
																			<a href="avoir.php">
																				...
																				<span>Créer un nouvel avoir</span>
																			</a>
																			...
																			<a href="<?php echo"$url&page=$pag&nombre=$nombre&validation=VALIDATION"; ?>">
																				...
																				<span>Voir les avoirs en attente de paiement</span>
																			</a>
																			<?php }else{ ?>
																			<a href="<?php echo"$url&page=$pag&nombre=$nombre&validation="; ?>">
																				<img src="images/icones/temps-qui-passe-icone-6367-32.png" width="24" height="24" align="middle" />
																				<span>Voir tous les avoirs</span>
																			</a>
																			<?php } ?>
																		</span>
																	</td>
																...
																<tr>
													                <td class="searchTD">
													                  ...
													                  <select name="interlocuteur" id="interlocuteur" class="searchSelect">
																	    <option value=""></option>
		<?php
			$iselect = mysql_query("select DISTINCT soco.INITIALE, soco._ID FROM avoir JOIN societe_contact soco WHERE avoir.ID_SOCIETE = " . $societeContact->societe->id . " AND soco.ID_SOCIETE = " . $societeContact->societe->id . " ");
		...
		<?php
			...
			$fselect = mysql_query("select DISTINCT a.FACTURE, f.REFERENCE from avoir a, facture f where a.ID_SOCIETE = " . $societeContact->societe->id . " AND a.FACTURE IS NOT NULL AND f._ID = a.FACTURE");
		...
		?>
																					...
																					<table width="100%">
																						...
																						<tbody>
																							...
																							<tr>
																								...
																								<th width="96" bgcolor="#5289BA"><span
																									class="Style1" style="font-size: 11px"> Num facture </span>
																								</th>
																							...
																							<tr>
																								...
																								<td bgcolor="#CDDDEB">
																									<div align="center">
																										<a href="<?php echo"$url&page=$pag&nombre=$nombre&sort=DATE_EMISSION "; ?>">
																											<img src="images/icones/icon-max.gif" width="17" height="16" />
																										</a>
																										<a href="<?php echo"$url&page=$pag&nombre=$nombre&sort=DATE_EMISSION DESC"; ?>">
																											<img src="images/icones/icon-min.gif" width="17" height="16" />
																										</a>
																									</div>
																								</td>
																								<td bgcolor="#CDDDEB">
																									<div align="center">
																										<a href="<?php echo"$url&page=$pag&nombre=$nombre&sort=REFERENCE"; ?>">
																											<img src="images/icones/icon-max.gif" width="17" height="16" />
																										</a>
																										<a href="<?php echo"$url&page=$pag&nombre=$nombre&sort=REFERENCE DESC"; ?>">
																											<img src="images/icones/icon-min.gif" width="17" height="16" />
																										</a>
																									</div>
																								</td>
																								...
																								<td bgcolor="#CDDDEB">
																									<div align="center">
																										<a href="<?php echo"$url&page=$pag&nombre=$nombre&sort=TITRE"; ?>">
																											<img src="images/icones/icon-max.gif" width="17" height="16" />
																										</a>
																										<a href="<?php echo"$url&page=$pag&nombre=$nombre&sort=TITRE DESC"; ?>">
																											<img src="images/icones/icon-min.gif" width="17" height="16" />
																										</a>
																									</div>
																								</td>
																								<td bgcolor="#CDDDEB">
																									<div align="center">
																										<a href="<?php echo"$url&page=$pag&nombre=$nombre&sort=TOTAL_HT"; ?>">
																											<img src="images/icones/icon-max.gif" width="17" height="16" />
																										</a>
																										<a href="<?php echo"$url&page=$pag&nombre=$nombre&sort=TOTAL_HT DESC"; ?>">
																											<img src="images/icones/icon-min.gif" width="17" height="16" />
																										</a>
																									</div>
																								</td>
			...
			<?php
				...
				if($idFacture != "ALL"){
					if(empty($idFacture)){
						$facture="(FACTURE IS NULL OR FACTURE = '')";
					} else {
				...
				$select=mysql_query("select _ID from avoir WHERE ID_SOCIETE = " . $societeContact->societe->id . " " . $quete . " " . $parametres);
				$selecti=mysql_query("select * from avoir WHERE ID_SOCIETE = " . $societeContact->societe->id . " " . $quete);
				$totp=mysql_num_rows($selecti);
				$toggle="";
				$totalpage=0.0;
				while($fetch=mysql_fetch_array($select)){
					$avoir = new Avoir();
					$avoir->findById($fetch['_ID']);
					$factAvoir = new Facture();
					if (!empty($avoir->idFacture)) {
						$factAvoir->findById($avoir->idFacture);
					}
					if($toggle&1){
						$bgcolor="#EAF0F7";
					}
			...
			?>
				...
				$interloc="select INITIALE from societe_contact where _ID=" . $avoir->idSocieteContact;
				$interloc1=mysql_query($interloc);
				$interloc2=mysql_fetch_array($interloc1);
				echo $interloc2['INITIALE'];
			?> </span></td>
																								<td><span style="font-size: 11px"> <?php $d = $avoir->dateEmission; echo date("d/m/Y", strtotime($d)); ?>
																								</span></td>
																								<td><span style="font-size: 11px"> <?php echo $avoir->reference; ?> 
																								</span></td>
																								<td><span style="font-size: 11px"> <?php if (!empty($avoir->idFacture)) { echo $factAvoir->reference; } ?> 
																								</span></td>
																								<!-- Tier  -->
																								<td align="left"><span style="font-size: 11px">
																										<?php
						  $requet="select * from societe_client where _ID='" . $avoir->idSocieteClient . "'";
						  ...
						  ?> </span></td>
																								<td align="left"><?php echo $avoir->titre; ?>
																								</td>
																								<td align="right"><span style="font-size: 11px">
																										<?php echo $avoir->getHTPriceRemiseFormat();
						  ?> </span></td>
																								<td align="left">
																									<div align="center">
																										<?php if(empty($avoir->validation)){ ?>
																										<a href="#" onclick="fex(<?php echo $avoir->id; ?>);">
																										...
																										<?php }else{  $d=$avoir->dateValidite; echo date("d/m/Y",strtotime($d));  } ?>
																									</div>
																								</td>
																								<td align="left">
																									<div align="center" style="font-size: 11px">
																										<?php if(empty($avoir->validation)){ ?>
																										<a
																											href="avoir.php?id=<?php echo $avoir->id; ?>"><img
																										...
																										<?php if(empty($avoir->validation)){ ?>
																										<a href="#"
																											onclick="ex(<?php echo $avoir->id; ?>);"><img
																							...
																							<?php
						$toggle++;
					 	$totalpage = $totalpage + $avoir->getHTPriceRemise();
						 } ?>
