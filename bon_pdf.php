...
//			$re = mysql_fetch_array($resul);
		    ...
		    $this->Cell(90,10,'BON DE COMMANDE',0,1,'C',true);
		    //Saut de ligne
			$this->SetFont('Arial','',9);
		    $this->Ln(2);
			//sous titre
			$d1= utf8_decode('Date émission : '.date('d/m/Y', strtotime($bon->dateEmission)));
			$this->Cell(100);
			$this->Cell(90,5,'Ref : '.$bon->reference,0,0,'R');
			$this->Ln(1);
			$this->Cell(100);
			...
			if ($pag !=1) {	 
				...
				//TITRE
				$this->Ln(5);
				$this->Cell(0,10,utf8_decode($bon->titre).' (suite)',1,0,'C');
				$this->Ln(10);
				//En-tête du tableau
...
//			$re = mysql_fetch_array($resul);
			...
			$pied = $societeContact->societe->piedBonCmd;
...
//	$re = mysql_fetch_array($resul);
	...
	$socClient->findByLogin($bon->idSocieteClient);
//	$sel = "select * from contact where _ID = '" . $bon->destinataire . "'";
//	$resul = mysql_query($sel);
//	$re = mysql_fetch_array($resul);
	...
	$contactBon = new SocieteContact();
	$contactBon->findById($bon->idSocieteContact);
//	$selectinter = "select * from interlocuteur where _ID = '" . $bon->idInterlocuteur . "'";
//	$resultinter = mysql_query($selectinter);
//	$rinter = mysql_fetch_array($resultinter);
	
	$pdf->Cell(0, 10, 'Votre interlocuteur : ' . utf8_decode($contactBon->prenom) . ' ' . utf8_decode($contactBon->nom), 0, 0, 'L');
	$pdf->Ln();

	//TITRE
	$pdf->MultiCell(0,5,utf8_decode($bon->titre),1,'C',false);
	$pdf->Ln(10);
	...
	$pdf->Ln();
	$pdf->SetFont('Arial','',7);
	$chaine = str_replace('€','euros', $bon->commentaire); 
	...
	//$pdf->Cell(100,5,utf8_decode($delai2),0,0,'L');
	if ($yyy[0]==$pdf->PageNo()) {
		$pdf->SetY($yyy[1]);
		if ($pdf->GetY()+40>$pdf->PageBreakTrigger) {
        	$pdf->AddPage();		
	...
	if (!empty($bon->remise) && $bon->remise != 0) {
		$pdf->Cell(115);
		$pdf->Cell(51,5,'TOTAL HT :',0,0,'L');
		$pdf->Cell(24,5,$bon->getHTPriceFormat(),0,0,'R');
		$pdf->Ln();
		$pdf->Cell(115);
		$pdf->Cell(51,5,'Remise globale '.$bon->getRemiseFormat().' % :',0,0,'L');
		$pdf->Cell(24,5,$bon->getTotalRemiseFormat(),0,0,'R');
		$pdf->Ln();
		$pdf->Cell(115);
		$pdf->Cell(51,5,'TOTAL HT NET :',0,0,'L',true);
		$pdf->Cell(24,5,$bon->getHTPriceRemiseFormat(),0,0,'R',true);
		$pdf->Ln();
	} else {
		$pdf->Cell(115);
		$pdf->Cell(51,5,'TOTAL HT :',0,0,'L');
		$pdf->Cell(24,5,$bon->getHTPriceRemiseFormat(),0,0,'R');
		$pdf->Ln();
	}		

	foreach ($allTva as $tva) {
		if ($tva->isActif || !empty($usedTva[$tva->id])) {
			$pdf->Cell(115);
			$pdf->Cell(51, 5, 'TVA ' . $tva->libelle . ' :', 0, 0, 'L');
			$pdf->Cell(24, 5, $bon->getTvaPriceFormat($tva->id), 0, 0, 'R');
			$pdf->Ln();
		}
	}
	...
	$pdf->Cell(24,5,$bon->getTTCPriceFormat(),0,0,'R',true);
	$pdf->Ln(10);
	$pdf->Cell(115);
	$pdf->Ln(10);
