...
//			$re = mysql_fetch_array($resul);
			...
			$this->Cell(90, 10, 'DEVIS', 0, 1, 'C', true);
			//Saut de ligne
			$this->SetFont('Arial', '', 9);
			$this->Ln(2);
			//sous titre
			$d1 = utf8_decode('Date émission : ' . date('d/m/Y', strtotime($devis->dateEmission)));
			$d2 = utf8_decode('Durée de validité : ' . $societeContact->societe->delai1);
			$this->Cell(100);
			$this->Cell(90, 5, 'Ref : ' . $devis->reference, 0, 0, 'R');
			$this->Ln(1);
			$this->Cell(100);
			...
			if ($pag !=1) {
				...
				//TITRE
				$this->Ln(5);
				$this->Cell(0, 10, utf8_decode($devis->titre) . ' (suite)', 1, 0, 'C');
				$this->Ln(10);
				//En-tête du tableau
...
//			$re = mysql_fetch_array($resul);
			...
			$pied = $societeContact->societe->piedDevis;
...
//	$re = mysql_fetch_array($resul);
	...
	$socClient->findByLogin($devis->idSocieteClient);
//	$sel = "select * from contact where _ID = '" . $devis->destinataire . "'";
//	$resul = mysql_query($sel);
//	$re = mysql_fetch_array($resul);
	...
	$contactDevis = new SocieteContact();
	$contactDevis->findById($devis->idSocieteContact);
//	$selectinter = "select * from interlocuteur where _ID = '" . $devis->idInterlocuteur . "'";
//	$resultinter = mysql_query($selectinter);
//	$rinter = mysql_fetch_array($resultinter);
	
	$pdf->Cell(0, 10, 'Votre interlocuteur : ' . utf8_decode($contactDevis->prenom) . ' ' . utf8_decode($contactDevis->nom), 0, 0, 'L');
	$pdf->Ln();

	//TITRE
	$pdf->MultiCell(0, 5, utf8_decode($devis->titre), 1, 'C', false);
	$pdf->Ln(10);
	...
	$pdf->Ln();
	$pdf->SetFont('Arial', '', 7);
	$chaine = str_replace('€', 'euros', $devis->commentaire);
	...
	if (!empty($devis->remise) && $devis->remise != 0) {
		...
		$pdf->Cell(24, 5, $devis->getHTPriceFormat(), 0, 0, 'R');
		$pdf->Ln();
		$pdf->Cell(115);
		$pdf->Cell(51, 5, 'Remise globale ' . $devis->getRemiseFormat() . ' % :', 0, 0, 'L');
		$pdf->Cell(24, 5, $devis->getTotalRemiseFormat(), 0, 0, 'R');
		$pdf->Ln();
		$pdf->Cell(115);
		$pdf->Cell(51, 5, 'TOTAL HT NET :', 0, 0, 'L', true);
		$pdf->Cell(24, 5, $devis->getHTPriceRemiseFormat(), 0, 0, 'R', true);
		$pdf->Ln();
	}
	else{
		$pdf->Cell(115);
		$pdf->Cell(51, 5, 'TOTAL HT :', 0, 0, 'L');
		$pdf->Cell(24, 5, $devis->getHTPriceRemiseFormat(), 0, 0, 'R');
		$pdf->Ln();
	}
	...
	foreach ($allTva as $tva) {
		if ($tva->id != 0.0 && ($tva->isActif || !empty($usedTva[$tva->id]))) {
			...
			$pdf->Cell(24, 5, $devis->getTvaPriceFormat($tva->id), 0, 0, 'R');
			$pdf->Ln();
		}
	}
	...
	$pdf->Cell(24, 5, $devis->getTTCPriceFormat(), 0, 0, 'R', true);
	$pdf->Ln(10);
	$pdf->Cell(115);
	$pdf->Cell(51, 5, 'Acompte TTC ' . $devis->getAcompteFormat() . ' % :', 0, 0, 'L');
	$pdf->Cell(24, 5, $devis->getAcomptePriceTTCFormat(), 0, 0, 'R');
	$pdf->Ln(10);
